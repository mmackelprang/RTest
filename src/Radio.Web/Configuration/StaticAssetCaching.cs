...
/// </para>
/// <para>
/// Restarting the browser is not itself a fix — Chrome's HTTP cache lives in the profile
/// directory and outlives the process, so the kiosk relaunch at the end of a deploy would
/// otherwise re-read the same stale entries. <c>Deploy-ToLinux.ps1</c> does separately delete
/// that cache directory for the kiosk profile, so the relaunch is not in fact reading stale
/// entries today; but that is a mitigation on one profile on one box, it does nothing for a
/// laptop or tablet on the LAN pointed at <c>radio:5002</c>, and nothing at all under
/// <c>-NoRestart</c>. It is left in place. Setting the header is what makes the guarantee hold
/// for every client rather than one, and it is what this type is for.
/// </para>
/// <para>
...
/// <c>fonts/*</c> are all served under stable names too. The one near-exception proves the point:
/// <c>App.razor</c> links <c>js/virtual-keyboard.js?v=2</c>, a query-string version added by hand in
/// PR #320 (its commit message says "add cache-busting query param") — a human-maintained buster, not
/// a content hash, and it only ever protected that one file. A long <c>max-age</c> on any of these
