...
public class AudioStateHubService : IAsyncDisposable
{
  ...
  /// <remarks>
  /// ⚠ internal for the test seam, as <see cref="Radio.Web.Services.AudioStateStore"/> is at its own
  /// :230-243, and for the reason it gives there: a field-like event cannot be raised from outside
  /// the type that declares it, so a test holding an AudioStateHubService cannot make
  /// RadioStateChanged fire. This class has a second reason of its own — the SignalR On&lt;T&gt;
  /// lambda that used to hold this body cannot be reached without a STARTED HubConnection, and every
  /// fixture in Radio.Web.Tests runs on OfflineHubTransport. Radio.Web.csproj:31 already declares
  /// InternalsVisibleTo("Radio.Web.Tests").
  /// </remarks>
