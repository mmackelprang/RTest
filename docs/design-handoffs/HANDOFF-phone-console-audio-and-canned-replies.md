...
**⚠ The consequence the owner must confirm.** With `＋ New` gone, **the console can only text people who have texted it first.** The `Text back` quick action — specced in the original handoff as owner question #5 (`:542`), still unanswered, still deferred in `FUTURE-WORK.md:593` and [`BUILDER_QUEUE_ARCHIVE.md`](../BUILDER_QUEUE_ARCHIVE.md) — becomes the *only* remaining way to start a thread with someone in your call history or voicemail. Feature C therefore **forces** that long-deferred question. It is raised as **Q1**, not decided here.
