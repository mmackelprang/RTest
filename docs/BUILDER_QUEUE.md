...
>
> **Last updated:** 2026-06-21 (Coordinator) — **GV Messages READ experience fully shipped.** GV-1 (foundation) ✅ PR #437, GV-2 (voicemail) ✅ PR #439, GV-3 (texts) ✅ PR #440 — merged in sequence; GV-3 reconciled onto GV-2's `FeedItem`/`FeedKind` projection (texts now interleave in the unified **All** feed) with a combined live UAT at 1920×720 (caught + fixed a latent filter-bar wrap that made "Calls" unclickable). Union suite **709 passed**. SMS **send + compose stay flagged OFF** (`RotaryPhone:Gv:SendEnabled`). **GV-4 🔒 → 📋** — both deps (GV-2 + GV-3) now met; contract ratified + stable (ADR-024), ships behind `RotaryPhone:Gv:MarkReadEnabled` (default off) when RotaryPhone's owner-HELD build deploys. NU1903 SQLite blocker cleared (ADR-023, PR #438).
