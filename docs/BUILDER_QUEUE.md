...
>
> **Last updated:** 2026-06-20 (Planner) — wrote the GV-4 plan (durable GV mark-read / read-state) against the now-ratified+stable contract (ADR-023) and un-held the row: GV-4 ⛔ → 🔒 (external blocker resolved; now depends only on GV-2 + GV-3). GV-4 ships behind `RotaryPhone:Gv:MarkReadEnabled` (default off) and lights up when RotaryPhone's owner-HELD build deploys. Earlier: queued the GV Messages (Voicemail + SMS) arc (PR1/PR2/PR3). GV-1 is in flight as PR #437.
