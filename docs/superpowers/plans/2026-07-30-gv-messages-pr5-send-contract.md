...
>
> 1. **Use `-TargetHost radio`, and note `Deploy-ToLinux.ps1` also defaults to a wrong architecture** — the box is `x86_64` while the script defaults to `linux-arm64`. Deploy with `-TargetHost radio -Runtime linux-x64` (tracked as **OPS-1 (c)**).
>    <br>**Corrected 2026-08-10:** this step previously read *"The box is `192.168.86.50`. `radio` and `piradio` do **not** resolve from WSL"* and told you to deploy with `-TargetHost 192.168.86.50`. That is **inverted and will fail** — the bare IP gives `Permission denied (publickey,password)` because `~/.ssh/config` scopes the box's `IdentityFile` to a `Host radio` block under `IdentitiesOnly yes`. `radio` resolves fine from WSL and is the working form. See `CLAUDE.md` → Deployment.
