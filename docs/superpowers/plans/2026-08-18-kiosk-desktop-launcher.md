...
| T15 | ⛔ **UNRUNNABLE AS WRITTEN — superseded by `KIOSK-3` (2026-09-09).** The step polled `PSIDTS=` against a `660 ≤ PSIDTS ≤ 1200` band; `--print-status` no longer emits `PSIDTS=` and RotaryPhone's #79 deletes the field. *Original: "GV blackout trough is never reported."* | **Replacement:** poll `--print-status` every 60 s for 25 min, logging `GV_AVAILABLE=` / `GV_COOKIESVALID=` / `GV_DEGRADED=` / `GV_AUTHBLACKOUT=` / `GV_LASTSUCCESS_AGE=` and `VOICE=` | Every sample with all four terms healthy and `GV_LASTSUCCESS_AGE ≤ 120` shows `VOICE=online` and `TIER=silent`. ⚠ The underlying rule (a routine self-clearing condition is never reported) is **unchanged**; only its instrument moved. **Discharged during `KIOSK-3`: 160 consecutive samples 8 s apart over 21 min** (14:44:30–15:05:50Z) returned **159 `online` / 1 amber**, max `GV_LASTSUCCESS_AGE` 59 s. The step asks for 25 min at 60 s spacing (25 samples); this is 21 min at 8 s spacing (160 samples), i.e. shorter in wall-clock and **6× denser**, and it spans a full ~20-minute RotaryPhone cookie cycle — which is the condition the step exists to sample. ⚠ The one amber (`cookiesValid:false` + `degraded:true`, one sample wide, under 16 s) is a **real** self-clearing condition being reported, so this step is passed on rate (~0.6%) rather than on zero. Re-run after RotaryPhone's #78 deploys, which they say removes the churn behind it. |
