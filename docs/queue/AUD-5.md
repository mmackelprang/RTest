...
|---|---|
| Status | 📋 |
| Plan | _plan TBD (small — a generation parameter, one await-free re-check, and an explicit decision on `IsInitialSync`; **the diagnosis is complete and must not be re-brainstormed**)_ |
