...
One user report — *"no audio from the soundbar"* — decomposed into **five distinct bugs**, which produced three PRs the same day: **#467 ✅** (systemd restart limiter made functional; `StartLimitIntervalSec` was inherited-and-unreachable on both units, plus crash-forensics groundwork), **#469 ✅** (cross-source metadata contamination — `TrackIdentified` was broadcast to every source, not just the active one — and a BT state clobber where deferred capture demoted a `Playing` source to `Ready` and silently killed fingerprinting), and **#468 🔵 open, awaiting an owner decision** (confirm the Cast output before leaving local output muted; make source teardown state-independent). **This section is the deferred remainder**, queued as eight rows in [`docs/BUILDER_QUEUE.md`](BUILDER_QUEUE.md) — scoped, not re-brainstormed.
...
| AUD-3 | _plan TBD (blocked)_ | **Close the Cast connect/teardown data race.** The connect path touches `GoogleCastOutput`'s unsynchronised `_client`/`_connectedReceiver` without holding `SoundFlowAudioEngine._activeOutputLock`, concurrently with teardown. **PR #468's post-`StartAsync` guard closes the observable outcome, not the race** — do not read its merge as closing this. Proper fix changes the engine's output API and wants hardware UAT. **🔒 Blocked on PR #468** (a PR, not a row — deviation recorded in the queue). | PR #468 | `fix/cast-connect-teardown-race` |
| AUD-4 | _plan TBD_ | **Unify the two source-removal layers and rename `SoundFlowMasterMixer`** — a `List<IAudioSource>`, not the audio mixer; the real detach is `SoundFlowPlaybackService.StopAsync` → `RemoveComponent`. Its log line **actively misled a previous fix attempt** (`03a6fea`, applied one layer too high, silently inert for months). ⚠ Trap: do **not** sweep by `oldSource.Id` — SDR registers under a different key; sweep `_activeComponents`. **📋 Queued — same root as AUD-2; prefer AUD-2 first.** | — | `refactor/unify-source-removal-and-rename-mixer` |
