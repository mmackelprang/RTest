...
| GV-7 | _plan TBD (design-led)_ | Render non-dialable SMS senders: thread-list row, conversation header, and contact resolution for **numeric short codes** and **opaque 36-char sender IDs**. There is **no `fromName` in the GV payload**, so names can only come from local contact resolution — which cannot resolve these senders. Display counterpart to GV-5's send-side gate. **📋 Queued — live observations incoming; do not design blind.** | GV-3 | `feat/gv-messages-pr7-nondialable-senders` |
