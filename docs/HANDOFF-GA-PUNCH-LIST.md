...
| **O10** | ⚠ **BLOCKED AT `ENC-11` BY A FIRMWARE DEFECT (2026-09-02).** `ENC-1` → `ENC-2` → ~~`ENC-11`~~ → `ENC-8` / `ENC-12` / `ENC-14`. **The device drops every host-to-device report**, so neither the config push nor the read-back verify can work. Proven with `0x05` Reset diagnostics, whose effect is observable: the counters did not move. Cause is in the C++ firmware — it has an interrupt OUT endpoint, so TinyUSB delivers output reports with `report_id == 0` and the real ID in `buffer[0]`, while `main_generic_hid.cpp:747,761` dispatches only on the parameter. Diagnosis and fix: `design/research/ENC-11-firmware-drops-output-reports.md`. **The fix is in the RotaryUsb repo, not this one.** `ENC-1` is unaffected — input reports arrive normally. | | The §7 startup-config arc is a chain, not a set. `ENC-2` supplies the report plumbing and the config model; `ENC-11` is the runtime push/verify loop built on it; the fault surfacing, the flash baseline and the diagnostics card all read state that only exists once `ENC-11` runs. Starting at any of the three tail items means inventing the state they display. |
