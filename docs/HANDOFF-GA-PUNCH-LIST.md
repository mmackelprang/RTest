...
> ⛔ **ID NAMESPACE — READ BEFORE ADDING A ROW. There is ONE `AUD-*` namespace, shared with
> [`BUILDER_QUEUE.md`](BUILDER_QUEUE.md). It was not always so.**
>
> **Reconciled 2026-09-09.** This document and the queue both numbered `AUD-*` independently. `AUD-1`
> through `AUD-9` happened to agree; **`AUD-10`, `AUD-11` and `AUD-12` meant three different things in
> each document**, so *"`AUD-12` shipped"* was simultaneously true of the queue row and false of the
> item of the same name here. Found by `AUD-12`'s own Builder while doing Docs Impact against the
> merged tree; one grep refuted the PR body's claim that the row had no punch-list entry.
>
> **The punch-list side was renumbered, because the queue side could not be**: those IDs have
> dossiers, branches, merged PRs and archive entries — immutable history. Every occurrence here was
> checked in context first to confirm none referred to the *queue's* items.
>
> | was (here) | now | subject |
> |---|---|---|
> | `AUD-10` | **`AUD-21`** | Confirm-or-close: BT disconnect-reason surfacing |
> | `AUD-11` | **`AUD-22`** | BT codec observability |
> | `AUD-11a` | **`AUD-22a`** | capture-node probe forks `pw-cli` when the listener is healthy |
...
> row.** ⛔ **CORRECTED 2026-09-09 (was: "the next free `AUD` number is `AUD-24` — the queue reached `AUD-20`"). THAT WENT STALE THE SAME DAY IT WAS WRITTEN**: the queue subsequently filed `AUD-24` (seek), `AUD-25` (phantom BT Turntable) and `AUD-26` (duck lost on source switch). **The next free `AUD` number is `AUD-32`** *(said `AUD-27` until 2026-09-25; the queue has since filed `AUD-27` … `AUD-31` — `AUD-30`/`AUD-31` are the phantom BT disconnect and eviction)*. ⚠ **Re-derive this from BOTH documents before claiming a number — do not trust this sentence either**, since its predecessor was falsified by its own author within hours: `grep -ohE 'AUD-[0-9]+' docs/BUILDER_QUEUE.md docs/BUILDER_QUEUE_ARCHIVE.md docs/HANDOFF-GA-PUNCH-LIST.md | sort -t- -k2 -n -u | tail -1` — ⚠ **and exclude this sentence and the queue's "next free" line from that grep**: both name a *free* number, so a naive run counts it as taken.
>
...
> number across BOTH — a number is not free because one file has not used it.**
...
>
> ⭐ **UPDATED 2026-09-25 — the phone arc's P0s have both shipped; the paragraph below is SUPERSEDED.**
> Checked against [`BUILDER_QUEUE_ARCHIVE.md`](BUILDER_QUEUE_ARCHIVE.md), not recalled: every `PHN-1` PR is
> archived ✅ — `PHN-1a` [#528](https://github.com/mmackelprang/RTest/pull/528) and `PHN-1b`
> [#534](https://github.com/mmackelprang/RTest/pull/534) (2026-09-02), `PHN-1c`
> [#556](https://github.com/mmackelprang/RTest/pull/556), `PHN-1d` [#558](https://github.com/mmackelprang/RTest/pull/558),
> `PHN-1e` [#561](https://github.com/mmackelprang/RTest/pull/561) and `PHN-1f`
> [#564](https://github.com/mmackelprang/RTest/pull/564) (all 2026-09-04) — and `PHN-2` (PR 6)
> [#566](https://github.com/mmackelprang/RTest/pull/566) merged 2026-09-05. The two remaining arc items shipped too:
> Feature C (PR 7 in [`ORDERING-NOTES.md`](queue/ORDERING-NOTES.md)) was resolved by deletion as `PHN-4`
> [#578](https://github.com/mmackelprang/RTest/pull/578) (2026-09-05, `D31`), and Feature B shipped as `PHN-3`
> [#598](https://github.com/mmackelprang/RTest/pull/598) (2026-09-07). ⚠ *(The arc's PR numbering is not consistent
> across documents — `ORDERING-NOTES` calls Feature C PR 7, while `PHN-3`'s dossier calls Feature B "the eighth and last
> PR". Every item is shipped either way; the numbering is not reconciled here.)* **So by the list §9 counts, `PHN-1`
> and `PHN-2`, no P0 is open** — ⚠ **§9's P0 cell still reads "2 open" and was NOT re-tallied by this pass**; its
> own note already flags that cell's arithmetic for Planner.
> ⚠ **`PHN-2` §3 U5 AS SPECIFIED — start a voicemail, TAP the bar — has never been run.**
> [`RESULT.md`](uat/2026-09-09-phn2-sound-uat/RESULT.md)'s 2026-09-10 correction records that the defect the owner
> found (`AUD-24`, now archived) was on the file-player **drag**, a different component, and that U5 must not be
> marked discharged until the voicemail-tap path is itself exercised. `AUD-24`'s plan scheduled it as its UAT **U2**;
> **no result for U2 is recorded anywhere in the repo** — the `AUD-24` UAT pass (`5e07559d`) records the drag only.
>
> 📝 **2026-09-25:** `AUD-10` + `AUD-11` shipped as one PR, [#663](https://github.com/mmackelprang/RTest/pull/663) —
> **owner UAT at the cabinet passed 3 of 3** pause/resume cycles the same day. `AUD-30` (phantom BT disconnect tears
> down a healthy stream) and `AUD-31` (phantom eviction) were filed in
> [#662](https://github.com/mmackelprang/RTest/pull/662). *(These are queue `AUD-*` IDs — see the namespace note
> above.)*
>
> _Superseded 2026-09-25 — kept as written:_ **P0 remaining: 2, both in the phone arc.** `PHN-1` is
...
> mirror-case queue)** — see [`ORDERING-NOTES.md`](queue/ORDERING-NOTES.md) § Dependency / ordering notes. `PHN-1e` is queued
...
| **O3** | RETRACTED ~~**`AUD-2` before `AUD-4`.**~~ | ⛔ **RETRACTED 2026-09-06 while planning `AUD-4`; do not honour it.** The premise was that `AUD-4` unifies the source-removal layers *on the assumption* that `AUD-2`'s key mismatch is real. It does not: `AUD-4`'s roster is a `List<IAudioSource>` keyed by **object reference**, so it reaches no conclusion about key identity and there is no key for it to wait on. The two rows are independent and either may be claimed first. Recorded in `queue/AUD-2.md`, `queue/AUD-4.md` and `ORDERING-NOTES.md`; this cell was the last copy still asserting it. ✅ Moot in any case since 2026-09-09 — `AUD-2` shipped. |
...
>
> ✅ **That paragraph was written on 2026-09-06 in the future tense of a fix that had not shipped. It
> shipped 2026-09-09 and the claim is now discharged**, so the two log shapes really do date a log
> excerpt to either side of it. ⚠ The `AudioManager.cs` line numbers cited above (`:481`, `:557-559`)
> describe the code as it stood BEFORE that fix and are deliberately left as they were — they are part
> of the historical finding, not a pointer into the current tree.
...
| ID | Item | Why it matters in the cabinet | Effort | Deps | Queued? |
|---|---|---|---|---|---|
| **`AUD-2`** | ✅ **SHIPPED 2026-09-09.** ~~Confirm-or-close: is SDR gain/ducking silently dead?~~ **It was, on four source types rather than the one this row named.** The confirm half closed 2026-09-05 (code read) and was then **confirmed BY EAR** on 2026-09-09, when the owner ran `PHN-2`'s nine `SOUND` checks at the cabinet and heard no ducking on two independent paths — a voicemail over the radio and a TTS text over the radio ([record](uat/2026-09-09-phn2-sound-uat/RESULT.md)). Fixed by registering under `IAudioSource.Id` at `SDRRadioAudioSource.cs`, `USBAudioSourceBase.cs` (inherited by `RadioAudioSource`, `VinylAudioSource`, `GenericUSBAudioSource`) and `FilePlayerAudioSource.cs`. ⚠ **The anchor this row carried was stale twice over** — `SDRRadioAudioSource.cs:908` had drifted to `:915` by 2026-09-06 and the line no longer exists. ⚠ **UAT IS NOT DISCHARGED.** Unit tests assert the mechanism on a registered component and a source-text lint pins the key; **neither can hear anything.** The audible check is `PHN-2` §3 U1 re-run at the cabinet — play a voicemail over the radio and hear the radio duck. | Ducking is a documented core pattern. Confirmed: the announcement played *over* the radio at full level instead of under it. | 0.5 d confirm + 1 d fix — **spent** | `TEST-1` | ✅ shipped |
...
| ID | Item | Why it matters in the cabinet | Effort | Deps | Queued? |
|---|---|---|---|---|---|
| **`PHN-3`** | **The SMS speak button (Feature B) — fully specified at handoff `:297-430`, never queued.** Gutter placement, inbound-only, always visible, no seek or pause, 1000-char cap, URLs → "a link", emoji dropped, never speak the timestamp, strip the MMS `+1XXXXXXXXXX - ` prefix, **keep digit runs verbatim** ("verification codes are the single most valuable thing this feature reads"), one voice at a time. `MessageBubble.razor` is **56 lines and has no speak button, not even a disabled placeholder.** | A console you can ask to read you a text from across the room is the feature that makes the phone surface make sense *in furniture*. Spec-complete; nearly all its cost is `PHN-1`. | 2–3 d | `PHN-1` (O6) | ✅ **SHIPPED 2026-09-07 — [#598](https://github.com/mmackelprang/RTest/pull/598).** Plan: [`PHN-3-the-sms-speak-button.md`](../design/plans/PHN-3-the-sms-speak-button.md); dossier: [`queue/PHN-3.md`](queue/PHN-3.md); archived in [`BUILDER_QUEUE_ARCHIVE.md`](BUILDER_QUEUE_ARCHIVE.md). §9's P1 *open* count moved **33 → 32** with this merge — and only with the merge. _(The note that stood here, that queueing does not move the counts, was correct and is retained as the reason the number did not move on 2026-09-05.)_ |
...
| **`docs/BUILDER_PROMPT.md` does not exist** — nor the "Handoff Protocol at the top of `BUILDER_QUEUE.md`" the Builder role points at | **Deliberately not a queue row** — but worth 20 minutes from the owner. | Already decided in the queue's [`ORDERING-NOTES.md`](queue/ORDERING-NOTES.md). Recorded here because every Builder dispatched at this project is being pointed at two documents that are not there. |
...
| **Deleting `.texts-compose` along with the rest of the compose CSS** | ⚠ **MUST NOT — recorded here because it is the obvious next move and it breaks the surface.** ✅ **NAVIGATED CORRECTLY BY `PHN-4` (2026-09-05): `.texts-compose` survives and now wraps the tier-3a pill; `5842-5859` was re-verified live against its five consumers and left untouched. Both warnings held up under a fresh reading of the file.** | `design-system.css:5928-5933` is the compose-bar **container**, used at `PhoneTextsPanel.razor:262` (the composer, which goes) **and `:275` (the degraded pill wrapper, which stays)**. `PHN-4` deletes `5916-5934` *except* this block. ⚠ **A related trap in the same file:** an earlier scoping pass cited the dead CSS as **`design-system.css:5842-5859`**. **That range is live** — `.skeleton-feed-chip`, `.msg-list` and `.msg-bubble`, all referenced from `MessageBubble.razor:8`, `PhoneTextsPanel.razor:36`/`:53`/`:131`/`:154` and `PhoneMessagesPanel.razor:580`. **Deleting it would break every message bubble and both loading skeletons.** The correct block is `5916-5934`. |
...
| **D29** | Are the knob bodies straight-sided all the way down to the panel face, so §9.5's engraving clearance holds? | ✅ **CLOSED — YES, straight-sided, and the clearance holds.** Rev 4's shop-floor finding went to the owner and came back confirmed; the handoff §9.5 records it as a **checked-and-closed constraint rather than a silent non-issue**, and that framing is the whole value of the entry. The column is tight: a label's top edge clears the knob above it by about **0.8 mm** — VOLUME's label occupies y 40.03–43.99 mm while a 15 mm VOLUME knob ends at y 39.25 mm, and TUNING is the same to within a tenth — **and that clearance assumes 15 mm all the way down.** ⚠ **SUPERSEDED 2026-09-05 BY THE REV-6 DRAWING — THE CLEARANCE IS NOW 0.31 mm, NOT 0.8 mm, AND THE OWNER HAS ACCEPTED IT.** The figures in the sentence above describe the 2026-09-01 revision. `design/hardware/front-panel-layout_4.svg` was **replaced in place** on 2026-09-05 with the rev-6 output of `front-panel-svg-generator-v12.html`: the knob column moved (`cx` 72 → 90, every `cy` up 18 units — vertical pitch unchanged at ~84), and the labels became **ALL CAPS and roughly double size**, which is what consumed the margin. Measured off the committed file with a method first validated against this entry's own 40.03–43.99 mm figures, all four now agree more tightly than before: **VOLUME 0.31, SOURCE 0.30, PRESETS 0.30, TUNING 0.31 mm** — labels at y 33.21 / 62.84 / 92.47 / 122.11 mm, 15 mm knobs ending at 32.90 / 62.53 / 92.17 / 121.80 mm. ⚠ **The filename still says `_4` while the content is rev 6.** That was the owner's explicit choice on 2026-09-05, taken so the eleven citations of this path across six documents keep resolving; the alternative was an eleven-site rename. **Do not trust the `_4` in the name to tell you which revision you are looking at — trust this entry and the file's own header comment.** So the 0.8 mm figure — now 0.31 mm, which makes the point harder, not softer — is the reason *"any 15 mm knob"* is **not** a safe substitution later, which is a thing the answer alone would not have told anyone. ⚠ **RENUMBERED FROM `D26` ON 2026-09-04 — read the note under *Closed 2026-09-03* for why this one moved and the eSpeak decision did not.** |
...
(✅ **discharged — it closed as CONFIRM on 2026-09-05 and shipped 2026-09-09**), `AUD-21`, `GV-10`, `TEST-2`, `UX-1`. **Down from seven to five on 2026-08-19:** `SEC-1` is decided by D15
...
| `design/plans/` `bt-codec-observability` (2026-05-22) | `AUD-22` |
| `design/plans/` `pw-event-subscription` (2026-05-22) | `AUD-23` |
| `docs/plans/2026-05-22-audio-thread-isolation.md` | `LOG-9`, `LOG-10` |
| `design/plans/2026-03-11-bt-disconnect-reason.md` | `AUD-21` |
