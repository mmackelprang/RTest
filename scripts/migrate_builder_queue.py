...
def build_index(rows: list[Row], legend: str, risks: str, sizes: dict[str, int]) -> str:
    ...
    out += [
        ...
        "",
        "The restructure moved content that other documents cite. Those citations were re-pointed "
        "in the same PR, **at the file rather than at a line number** — a line anchor cannot "
        "survive the next edit, a dossier path can. `design/plans/*`, `design/decisions/*`, "
        "`docs/design-handoffs/*`, `docs/superpowers/plans/*`, `ROADMAP.md`, `FUTURE-WORK.md`, "
        "`INTEGRATIONS.md` and `HANDOFF-GA-PUNCH-LIST.md` now link to `queue/<ID>.md`, "
        "`queue/ORDERING-NOTES.md`, `queue/CROSS-REPO-HANDOFFS.md`, `queue/FAST-FOLLOWS.md` or "
        "`BUILDER_QUEUE_ARCHIVE.md` as appropriate.",
        "",
        "⚠ **Cite a row as `queue/<ID>.md`, not as `BUILDER_QUEUE.md:<line>`.** The old form is "
        "what the restructure had to go back and repair in 21 files.",
        "",
        "Two citations were deliberately left alone, both recorded rather than guessed:",
        "",
        "- **`design/plans/AUD-2-one-key-per-source.md:3`** — its sentence is *about* line numbers "
        "(*\"`:129` on `main`, `:131` in this working tree\"*), so re-pointing it needs a rewrite, "
        "not a path swap. It was **already wrong before this change** — `:129` was `AUD-7`'s row.",
        "- **`src/Radio.Web/Services/ApiClients/GvResult.cs:41`** — an XML doc comment citing "
        "`§ Dependency / ordering notes`. Correcting it means touching `src/`, which this "
        "documentation change deliberately does not.",
        "",
