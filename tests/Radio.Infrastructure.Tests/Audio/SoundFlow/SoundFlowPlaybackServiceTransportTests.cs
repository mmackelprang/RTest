...
public class SoundFlowPlaybackServiceTransportTests
{
  ...
  private static SoundFlowPlaybackService CreateService()
  {
    ...
    // SoundFlowAudioEngine's constructor only stores its collaborators and subscribes to two
    // of their events — the MiniAudio device is not created until InitializeAsync, which no
    // test here calls. The same construction is already done by SoundFlowAudioEngineTests.
