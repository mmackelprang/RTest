...
/// eleven log statements this row fixed is written again.
/// </summary>
/// <remarks>
/// ⚠⚠ <b>WHAT THIS TEST CANNOT DO, AND IT MATTERS MORE THAN WHAT IT CAN.</b>
/// <b>This is a regression lint over eleven known shapes. It is NOT a proof of the property.</b>
...
/// of these lines in six months, having never read <c>TTS-11</c>. The eleven shapes it knows are
/// the eleven that actually occurred. The real coverage of the PROPERTY lives in the tests that
...
/// <c>Radio.Infrastructure.Tests</c>) and the two controller pins in <c>Radio.API.Tests</c>.
///
/// ⚠ <b>Two shapes from the plan's list are NOT enforced globally, and one is not enforced at
/// all.</b> Both decisions are recorded here rather than made silently, because a lint tuned until
/// it is green can be tuned until it is worthless.
///
/// <list type="bullet">
/// <item><b><c>source.Name</c> is scoped to the mixer.</b> Fifteen live call sites log an
/// <c>IAudioSource</c>'s <c>Name</c> and every one of them is SAFE: they are on the primary-source
/// path, where every implementation returns a constant ("Bluetooth Audio", "File Player",
/// "Radio (RF320)"…). A global rule would flag fifteen correct lines and be deleted within a week.
/// It is enforced in <c>SoundFlowMasterMixer.cs</c>, which is the domain-agnostic bookkeeping code
/// that has no idea whether it is holding speech — which is exactly why it leaked.</item>
