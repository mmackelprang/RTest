...
3. **File-player seek WAS display-only (✅ fixed by `AUD-24`, 2026-09-10); position is still a wall-clock estimate**, although the SoundFlow version in use (`1.*` → 1.4.1) exposes `SoundPlayerBase.Seek()/Time/Duration`. Comments claiming the API doesn't exist are stale. ⚠ **Only the first half is fixed.** `FilePlayerAudioSource.Position` still reports a `+1s`-per-tick accumulator advanced by `MonitorPlaybackAsync` rather than reading `SoundFlowPlaybackService.GetPosition`, so it drifts; `AUD-24` deliberately did not change that, because `_position` is also the persisted resume position and the completion deadline. Carried forward in that plan's § 6.
