...
## ADR-029 Amendment 2: D7's two client-side stop conditions were falsified on the appliance

**Date:** 2026-09-04
**Status:** Accepted for the two corrections; **one blocking owner decision is OPEN** (ADR §16.3)
**Amends:** [ADR-029](decisions/2026-08-03-gv-audio-through-engine.md) **D7 §7.3 and §7.5**. Nothing else in ADR-029 changes — D1-D6 and D8-D10 stand, and **§7.1's max-duration cap is verified on hardware and untouched.**
**Context:** `PHN-1e` ([#561](https://github.com/mmackelprang/RTest/pull/561)) implemented D7's three stop conditions and measured them on `radio`. Two work. Two ADR claims do not, and the Builder blocked the row rather than merging — which is what the plan's own open assumption U3 instructed.

**Falsification 1 — §7.3's circuit-lifetime premise is inverted, and the rule has no true-positive path.**
§7.3 asserts *"Blazor Server does not tear a circuit down at tab close but after the disconnect retention window (default ~3 minutes)."* The opposite is documented: a **graceful** termination — reload, tab close, navigating away — releases the circuit **immediately**; `DisconnectedCircuitRetentionPeriod` (10 minutes here) covers **non-graceful** drops only, and the client-side trigger is the `unload` event. So the rule's whole story — circuit B is already open when circuit A times out three minutes later — is backwards: the old document must unload before the new one loads. **Measured twice on the box at `dd8e85ec`: a single-browser reload is `1 → 0 → 1`, close before open**, and the backstop stopped a 49-second recording **7 seconds in**, ~0.4 s before the replacement circuit arrived. Two tabs measure `2 → 1 → 2` and nothing stops — which is why the defect is invisible in the multi-client case and destructive in the single-kiosk case that is this appliance's normal state. **Every reachable firing is a false positive or is dead behind the 300 s cap**, so this is a design defect, not a latency to tune. A deploy relaunches the kiosk and therefore silences attended playback the same way.

**The corrected model, and the sentence §7.3 should have contained.** A circuit is server-side component state for one *document*, not a viewer: it outlives the viewer by up to 10 minutes on a drop and predeceases the viewer on every reload. `OnCircuitClosedAsync` is therefore **bimodal — immediate or ten minutes — and carries no indication of which**. `OnConnectionUpAsync`/`OnConnectionDownAsync` track the SignalR connection and are prompt in both cases, so they are a better proxy, but they do not escape the underlying fact: **"no client is attached" is not instantaneously observable — it is only observable over an interval**, because at the instant the last client leaves, a reload and a departure are identical and only the next few seconds distinguish them. Any rule that acts on the edge is wrong regardless of which edge it counts. Two implementation properties are correct and must survive any replacement: the handler is **singleton** (framework-documented, so the count is process-wide) and it acts on the **transition** to zero, never an observed zero (a `Radio.Web` restart leaves the count at zero at rest with audio in the room).

**⚠ OPEN OWNER DECISION — ADR §16.3, what should a refresh do?** Three framed options: **(1)** accept today's behaviour (a refresh stops playback — zero work, but the mechanism does its job never, and every deploy silences the room); **(2)** a grace period keyed on the *connection* rather than the circuit (the only option that also fixes the 10-minute drop case, but `N` is unmeasured and fails in the bad direction on a box with known-poor timing); **(3) ⭐ recommended — playback is not client-bound at all**: delete §7.3's stop, keep the handler for §8.1's seed, and let the stop conditions be an explicit stop, the `/sleep` rule, and the cap. Option 3 is recommended because it is correct today with nothing to tune, and because **the exposure it accepts was already accepted** — §15 lists "audio can play with no client attached, bounded by `MaxPlaybackSeconds`" as an owner-accepted cost of Amendment 1. §7.3 recorded dropping the rule as *"viable but worse"*; **that judgement was made against a rule believed to work** and does not survive the measurement.

**Falsification 2 — §7.5's `/sleep` rule never reaches the idle timer, which is the case it was written for.**
§7.5 claims *"`idle-dimmer.js` drives the same path through `OnJsSleepRequested`."* It does not. The idle timer calls `navigateToSleep('idle')`, whose body is `window.location.href = '/sleep'` and which carries its own comment saying it deliberately does **not** call `SetSleepAsync` because idle navigation must not pause playback. `OnJsSleepRequested` is reached only from `enterSleep()`, whose comment says *"not from idle"* — and **`enterSleep` has zero callers in the tracked tree**, because both the Sleep pill and the server push use `NavigationManager.NavigateTo("/sleep")`. So `SleepService.IsSleeping` is **false** on the idle path (already recorded as `docs/HANDOFF-NEXT-SESSION.md` gotcha 9), and the shipped rule — hooked to `EnterSleepAsync` — cannot fire there. ⚠ **The claim propagated through four documents** — ADR §7.5 → plan constraint C-51 → the shipped `SleepService` comment → the PR body — and not one of them checked `idle-dimmer.js`, which contradicts it in a comment written for this exact reason.

**Decision — how `/sleep` must be detected: two edges, not one, and neither is `IsSleeping`.** The seam already existed: `ENC-6`'s **`ISleepService.IsSleepScreenVisible`**, reported by `Sleep.razor` from `OnAfterRenderAsync(firstRender)`, whose own contract doc says all three ways of reaching the route produce the same server-side fact. **`SetSleepScreenVisible(true)`** expresses *"a client has the no-transport surface on screen"* and covers the pill, the idle timer, the server push and a direct navigation; **`EnterSleepAsync`** expresses *"the room is being parked"* and is the only one reachable with **no browser at all** (encoder long-press, API). Both stop attended playback; they are not redundant. In the repo's own vocabulary: **stop when `ConsoleWakeState` leaves `Awake`** — but as an **edge at the two write sites, never as a polled predicate**, because `WakeState` deliberately reads `Awake` while a wake claim is outstanding. C-51's other half stands unchanged and is why this must be a **stop, not a mute**: `WakeAsync` restores the pre-sleep mute state, so a muted-but-playing voicemail becomes audible **mid-word** when someone touches the panel.

**Consequences for `PHN-1e`.** The row **proceeds**; under the recommended option it needs **no re-plan**. Verified on the box and not re-litigated: the hub broadcast with string enums (C-47/U1), the store cache and one-shot seed (§8.1), the singleton `CircuitHandler` and its count (U2/C-52), and the max-duration cap as a dispatching `TimeProvider` timer (C-49). Two changes, both small and both in this row: remove §7.3's stop from `AttendedPlaybackCircuitHandler` (keeping the class, the seed and the count as diagnostics), and add the `SetSleepScreenVisible(true)` edge beside the existing `EnterSleepAsync` hook. ⛔ **Under Option 2 the row DOES need re-planning** — new callbacks, a timer, a config key and an on-box measurement to choose `N`. **Nothing moves to `PHN-1f`** (a `Radio.Core` ducking change; burying an unrelated edit there inverts the argument that split PR 5) **or to PR 6** — though PR 6 is the true deadline, because it is what makes the defect reachable by a user. Two new open questions: **Q11** (a *physical* stop would dissolve §7.3 and §7.5 both — sleep/encoder arc) and **Q12** (`IsSleepScreenVisible` is last-writer-wins across clients; recommended default *stop*).

**The lesson, because it is the second occurrence.** Both falsified rules are statements about **client lifetime**, and both were written from reasoning rather than measurement — Amendment 1 rewrote §7.3 to fix a failure it had reasoned into existence, and Amendment 2 rewrites it to fix a failure it reasoned away. Anything further in D7 gets measured on the box before it is written down.
