...
- **Two-flag distinction.** `RotaryPhone:Gv:MarkReadEnabled` is **our consumer flag** (gates whether our seam calls the route). RotaryPhone's server-side `EnableMarkRead` **build flag** gates whether their route exists. They are independent — confirm the route ships before flipping ours.
