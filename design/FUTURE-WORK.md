...
**Priority:** Medium — read experience (incl. durable read-state) ships today behind flags.

> ⛔ **SEND IS NOT DEFERRED WORK. IT IS REFUSED WORK — owner decision `D31` (2026-09-05), punch list §7.**
> Read this before anything below that describes send as pending. `/phone` is a **read surface**; SMS sending is
> never turned on. **`PHN-4` (2026-09-05) deleted the send path outright** — the composer, the new-recipient mode,
> `GvBridgeSendService`, its four exception types, its DI registration, and the `SendSmsRequest`/`SendSmsResponse`
> DTOs listed under *What's Implemented* below. The conversation pane now renders the amber
> **`Replies are turned off.`** pill in the compose slot. **`GV-5` is PARKED 🚫.**
> **This section's phrase *"one config flip + the endpoint" is now false*** and is kept only so the correction is
> legible. [ADR-028](decisions/2026-07-30-gv-sms-send-contract.md) is the surviving record of the wire contract and
> is the reconstruction path if `D31` is ever reversed.
> ⚠ **What is NOT affected:** mark-read (`GV-6`) is a different flag on different routes, and the `Text back`
> quick action below is a *read-surface* entry point that D31 does not settle either way.

### What's Implemented (PR1)

- DTOs: `VoicemailItemDto`/`VoicemailListDto`, `SmsMessageDto`/`SmsThreadDto`/`SmsThreadListDto`/`SmsThreadMessagesDto`, `GvDirection` helper (`src/Radio.Web/Models/ApiModels.cs`). ⚠ `SendSmsRequest`/`SendSmsResponse` were here and are **deleted by `PHN-4`** (`D31`).
...
- `src/Radio.Web/Components/Pages/PhoneTextsPanel.razor` — **PR3** thread list + conversation. ⚠ **The compose bar and new-recipient composer were deleted by `PHN-4`**; the compose slot now holds the `Replies are turned off.` pill. There is no `GvBridgeSendService` any more.
