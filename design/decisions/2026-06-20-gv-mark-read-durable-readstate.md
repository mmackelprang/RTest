...
| `409 Conflict` | **RotaryPhone's `GVBridge:EnableMarkRead` is `false` (feature dark).** Checked **first**, before any validation — **no GV call is made.** | `{ "error": "markread_disabled" }` | Recognised and latched (GV-6): one Warning, then mark-read is suppressed until `radio-web` restarts. Optimistic flip kept; no user-visible change. |
